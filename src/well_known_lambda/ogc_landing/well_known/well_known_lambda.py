...
def _prepare_landing_html_body(item: CatalogRecord, original_resource: str) -> str:
    ...
    body = '\r'.join([
        ...
        f'      .footnote {{ font-size: 0.9em; margin-top: 30px; border-top: 1px solid #ddd; padding-top: 10px; }}\r'
        f'      .footnote ol {{ padding-left: 20px; }}\r'
        f'      sup {{ font-size: 0.7em; vertical-align: super; }}\r'
...
def _prepare_well_known_html(items: List[CatalogRecord], protocol: str) -> str:
    ...
    return (
        ...
        '  <head>\r'
        '    <style>\r'
        '      body { font-family: Arial, sans-serif; margin: 0; padding: 20px; }\r'
        '      header, nav, section, footer { margin-bottom: 20px; }\r'
        '      header { background-color: #f5f5f5; padding: 10px; }\r'
        '      nav { background-color: #eee; padding: 10px; }\r'
        '      .content { padding: 20px; border: 1px solid #ddd; }\r'
        '      footer { text-align: center; font-size: 0.8em; color: #666; }\r'
        '      th.left {text-align: left}\r'
        '      th, td {padding: 5px}\r'
        '      td.top {vertical-align: top}\r'
        '    </style>\r'
        "    <title>Michael's Portfolio Listing of API Documentation Endpoints</title>\r"
        '  </head>\r'
        '  <body>\r'
        '    <header>\r'
        "      <h1>Michael's Portfolio Listing of API Documentation Endpoints</h1>\r"
        '    </header>\r'
        '    <nav><a href="/">Home</a> &gt; API Catalog</nav>\r'
        '    <section class="content">\r'
        '      <table>\r'
        f'{'\r'.join([
            '        <tr>\r'
            f'         <th colspan="3" class="left">\r'
            f'           <a href="https://{domain.domain}/index.html">{domain.title}</a>: {domain.description}\r'
            f'         </th>\r'
            '        </tr>\r'
            f'{'\r'.join([
                '        <tr>'
                '          <td class="top">Service API Documentation</td>\r'
                f'         <td class="top"><a href="{protocol}://{item.domain}{item.anchor}'
                f'{item.relations['service-doc']['href']}" rel="service-doc">'
                f'{item.relations['service-doc']['title']}</a></td>\r'
                f'         <td class="top">{'<br>'.join(item.relations['service-doc']['types'])}</td></tr>\r'

                '        <tr>\r'
                '          <td class="top">Service API Specification</td>\r'
                f'         <td class="top"><a href="{protocol}://{item.domain}{item.anchor}'
                f'{item.relations['service-desc']['href']}" rel="service-desc">'
                f'{item.relations['service-desc']['title']}</a></td>\r'
                f'         <td class="top">{'<br>'.join(item.relations['service-desc']['types'])}</td>\r'
                '        </tr>\r'

                '        <tr>\r'
                '          <td class="top">Service Conformance Metadata</td>\r'
                f'         <td class="top"><a href="{protocol}://{item.domain}{item.anchor}'
                f'{item.relations['service-meta']['href']}" rel="{item.relations['service-meta']['rel']}">'
                f'{item.relations['service-meta']['title']}</a></td>\r'
                f'         <td class="top">{'<br>'.join(item.relations['service-meta']['types'])}</td>\r'
                f'       </tr>\r'
        ...
        '    </section>\r'
        '    <footer>\r'
        f'     &copy; {current_year} Michael Leedahl\r'
        '    </footer>\r'
        '  </body>\r'
        '</html>'
    )
